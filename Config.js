 ...
 * Este bloque centraliza el mapeo entre el XML del SII y nuestra Bóveda Forense.
 */
const CONFIG_DETALLE_COMPRAS = {
  // 1. MAPEADO DIRECTO DE TAGS SII (DTE CHILE)
  SII_MAPPING: {
    SKU: 'VlrCodigo',       // Dentro de <CdgItem>
    NOMBRE: 'NmbItem',      // Tag obligatorio
    DESCRIPCION: 'DscItem', // Tag de texto libre (donde buscaremos el Lote)
    CANTIDAD: 'QtyItem',    // Cantidad física
    PRECIO: 'PrcItem',      // Precio unitario neto
    UNIDAD: 'UnmdItem',     // Unidad de medida (Kg, Un, Mt)
    MONTO: 'MntoItem'       // Monto total de la línea
  },

  // 2. REGLAS DE PARSEO SECUNDARIO (INTELIGENCIA FORENSE)
  EXTRACTORES: {
    // Busca el Lote en el campo Descripción
    LOTE: {
      pattern: /(?:LOTE|LOT|L:|B:|BATCH)\s*[:#-]?\s*([A-Z0-9.-/]+)/i,
      target_field: 'DESCRIPCION'
    },
    // Nota: Los alérgenos se comparan contra la lista de SYS_CONFIG
  },

  // 3. ESTRUCTURA ATÓMICA PARA DETALLE_JSON
  // Este es el contrato de salida para nuestra columna en la BDD
  SCHEMA_JSON: {
    sku: null,
    nombre: null,
    cantidad: 0,
    precio: 0,
    unidad: 'UN',
    lote: 'S/L',         // S/L = Sin Lote detectado
    alergenos: 'OK',      // 'OK' o 'CRÍTICO'
